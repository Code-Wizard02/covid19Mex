...
with st.expander("Detalles del análisis por rango de edad"):
    ...
    max_hombres = (
        conteo[conteo["SEXO"] == "Hombres"]
        .sort_values("CASOS", ascending=False)
        .iloc[0]
    )
    max_mujeres = (
        conteo[conteo["SEXO"] == "Mujeres"]
        .sort_values("CASOS", ascending=False)
        .iloc[0]
    )
...
for bar in ax2.patches:
    ...
    if height > 0:
        ...
        ):  # Solo mostrar valores significativos
            ax2.text(
                bar.get_x() + bar.get_width() / 2.0,
                bar.get_height() + 5,
                f"{int(height):,}",
...
with st.expander("Detalles del análisis por tipo de atención"):
    # Calcular algunos estadísticos interesantes
    total_ambulatorios = conteo_tp[conteo_tp["TIPO_PACIENTE"] == "Ambulatorios"][
        "CASOS"
    ].sum()
    total_hospitalizados = conteo_tp[conteo_tp["TIPO_PACIENTE"] == "Hospitalizados"][
        "CASOS"
    ].sum()

    # Encontrar el rango de edad con más casos para cada tipo
    max_ambulatorios = (
        conteo_tp[conteo_tp["TIPO_PACIENTE"] == "Ambulatorios"]
        .sort_values("CASOS", ascending=False)
        .iloc[0]
    )
    max_hospitalizados = (
        conteo_tp[conteo_tp["TIPO_PACIENTE"] == "Hospitalizados"]
        .sort_values("CASOS", ascending=False)
        .iloc[0]
    )
    ...
    porc_ambulatorios = (
        (total_ambulatorios / total_casos) * 100 if total_casos > 0 else 0
    )
    porc_hospitalizados = (
        (total_hospitalizados / total_casos) * 100 if total_casos > 0 else 0
    )
    ...
    with col1:
        st.metric(
            ...
            (
                f"{(total_ambulatorios/total_hospitalizados):.2f}"
    ...
    with col2:
        ...
        edad_prom_hosp = (
            sum(
                rangos_edad[row["RANGO_EDAD"]] * row["CASOS"]
                for _, row in conteo_tp[
                    conteo_tp["TIPO_PACIENTE"] == "Hospitalizados"
                ].iterrows()
            )
        ...
        edad_prom_amb = (
            sum(
                rangos_edad[row["RANGO_EDAD"]] * row["CASOS"]
                for _, row in conteo_tp[
                    conteo_tp["TIPO_PACIENTE"] == "Ambulatorios"
                ].iterrows()
            )
            / total_ambulatorios
            if total_ambulatorios > 0
            else 0
        )
...
conteo = (
    df_antigeno.groupby(["MES", "RESULTADO_ANTIGENO"])
    .size()
    .reset_index(name="PACIENTES")
)
pivotado = conteo.pivot(
    index="MES", columns="RESULTADO_ANTIGENO", values="PACIENTES"
).fillna(0)
...
for i, col in enumerate(col_resultados):
    ...
    umbral = (
        max(pivotado[col]) * 0.1 if max(pivotado[col]) > 0 else 0
...
ax.set_facecolor("#1e1e2f")
ax.set_title(
    "Evolución Mensual por Resultado de Antígeno", fontsize=16, pad=20, color="white"
)
...
with st.expander("Análisis detallado de resultados de antígeno"):
    ...
    with col1:
        ...
        pie_labels = [
            resultados_nombres.get(c, f"Resultado {c}")
            for c, v in totales_por_resultado.items()
            if v > 0
        ]
        pie_colors = [
            colores_resultado.get(c, "#ffffff")
            for c, v in totales_por_resultado.items()
            if v > 0
        ]
    ...
    with col2:
        ...
        # Renombrar columnas
        tabla_evol.columns = [
            resultados_nombres.get(c, f"Resultado {c}") for c in tabla_evol.columns
        ]

        # Mostrar tabla
        st.dataframe(
            tabla_evol.style.background_gradient(
                cmap="viridis",
                axis=None,
                vmin=0,
    ...
    if 1 in pivotado.columns and 2 in pivotado.columns:
        ...
        ax_pos.set_title(
            "Tasa de Positividad Mensual (Positivos/Total de Pruebas)",
            fontsize=14,
            color="white",
        )
        ...
        for i, porc in enumerate(positividad):
            if porc > 0:
                ax_pos.annotate(
                    ...
                    fontsize=9,
                    bbox=dict(
                        boxstyle="round,pad=0.3", fc="#222244", alpha=0.7, ec="gray"
                    ),
                )
...
df_intubado = df[
    df["FECHA_INGRESO"].notna() & df["INTUBADO"].isin([1, 2, 97, 98, 99])
].copy()
...
# Contar mensual
conteo_intubado = (
    df_intubado.groupby(["MES", "INTUBADO"]).size().reset_index(name="PACIENTES")
)
pivot_intubado = conteo_intubado.pivot(
...
ax.set_facecolor("#1e1e2f")
ax.set_title(
    "Evolución Mensual de Pacientes por Estado de Intubación",
    fontsize=18,
    pad=20,
    color="white",
)
...
for col in orden_columnas:
    for mes, valor in enumerate(pivot_intubado[col], 1):
        if (
            valor > max(pivot_intubado[col]) * 0.3
        ):  # Solo mostrar valores significativos (30% del máximo)
...
with st.expander("Análisis detallado de pacientes intubados"):
    ...
    if 1 in pivot_intubado.columns and 2 in pivot_intubado.columns:
        ...
        with col2:
            ...
            ax_pie.axis("equal")
            ax_pie.set_title(
                "Distribución de Pacientes por Estado de Intubación",
                color="white",
                fontsize=14,
            )
        ...
        ax_prop.set_title(
            "Porcentaje de Pacientes Intubados por Mes", fontsize=14, color="white"
        )
        ...
        tabla_meses["% Intubados"] = tabla_meses["% Intubados"].apply(
            lambda x: f"{x:.1f}%"
        )
...
with st.expander("Detalles del análisis de pacientes intubados"):
    ...
    with col2:
        ...
        ax_pie.axis("equal")
        ax_pie.set_title(
            "Distribución de Pacientes por Estado de Intubación", color="white"
        )
...
with st.expander("Análisis detallado de la distribución por edad"):
    ...
    jovenes = sum(
        conteo_rangos[rango] for rango in conteo_rangos.index if rangos_dict[rango] < 30
    )
    adultos = sum(
        conteo_rangos[rango]
        for rango in conteo_rangos.index
        if 30 <= rangos_dict[rango] < 60
    )
    mayores = sum(
        conteo_rangos[rango]
        for rango in conteo_rangos.index
        if rangos_dict[rango] >= 60
    )
...
with st.expander(f"Análisis detallado de comorbilidades en {tipo_comparacion.lower()}"):
    ...
    with col2:
        ...
        # Calcular porcentaje
        porc_con_comorb = (
            (tiene_alguna / total_pacientes) * 100 if total_pacientes > 0 else 0
        )
...
with st.expander("Análisis detallado por clasificación"):
    ...
    with col1:
        ...
        pie_labels = [
            nombres_clasif.get(c, f"Clase {c}") for c in totales_por_clasif.index
        ]
        pie_colors = [
            colores_clasif.get(c, "#ffffff") for c in totales_por_clasif.index
        ]
        ...
        ax_pie.axis("equal")
        ax_pie.set_title(
            "Distribución Total por Clasificación", color="white", fontsize=14
        )
    ...
    with col2:
        ...
        # Renombrar columnas
        tabla_evol.columns = [
            nombres_clasif.get(c, f"Clase {c}") for c in tabla_evol.columns
        ]

        # Mostrar tabla
        st.dataframe(
            tabla_evol.style.background_gradient(
                cmap="viridis", axis=None, vmin=0, vmax=max(totales_por_clasif)
            ).format("{:,.0f}")
        )
...
with st.expander("Detalles del análisis por rango de edad"):
    ...
    pivot_edad = conteo_edad.pivot(
        index="RANGO_EDAD", columns="ESTADO_INTUBADO", values="PACIENTES"
    ).fillna(0)
    ...
    with col2:
        ...
        ax_line.set_title(
            "Porcentaje de Intubación por Rango de Edad", fontsize=14, color="white"
        )
...
with st.expander("Análisis detallado de comorbilidades en fallecidos"):
    ...
    with col2:
        ...
        promedio_comorb = (
            total_comorbilidades / n_fallecidos_con_comorb
            if n_fallecidos_con_comorb > 0
            else 0
        )
        ...
        if df_confirmados is not None and len(df_confirmados) > 0:
            ...
            for col in cols_comorb:
                if col in df_confirmados.columns:
                    porc_general = (
                        df_confirmados[df_confirmados[col] == 1].shape[0]
                        / len(df_confirmados)
                    ) * 100
                    porc_fallecidos = (
                        (df_def[df_def[col] == 1].shape[0] / total_fallecidos) * 100
            ...
            diferencias_porc = dict(
                sorted(diferencias_porc.items(), key=lambda item: item[1], reverse=True)
            )
